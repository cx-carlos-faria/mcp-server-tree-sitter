...
def test_symbol_extraction_diagnostics(test_project: Dict[str, Any]) -> None:
...
def test_dependency_analysis_diagnostics(test_project: Dict[str, Any]) -> None:
...
def test_symbol_extraction_with_ast_access(test_project: Dict[str, Any]) -> None:
    ...
    def extract_symbols_manually(node: Any, path: tuple[int, ...] = ()) -> None:
...
def test_query_based_symbol_extraction(test_project: Dict[str, Any]) -> None:
    """
    ...
    try:
        ...
        def process_capture(captures: Any, target_type: str, result_dict: Dict[str, Dict[str, Any]]) -> None:
        ...
        def process_import_capture(captures: Any) -> None:
            # Check if it's returning a dictionary format
            if isinstance(captures, dict):
                # Dictionary format: {capture_name: [node1, node2, ...], ...}
                for capture_name, nodes in captures.items():
...
def test_debug_file_saving(test_project: Dict[str, Any]) -> None:
